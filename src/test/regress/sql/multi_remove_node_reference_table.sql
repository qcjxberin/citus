...
SELECT master_remove_node('localhost', 5555);
...
-- remove a node with reference table with active placement
CREATE TABLE remove_node_reference_table(column1 int);
SELECT create_reference_table('remove_node_reference_table');
SELECT master_remove_node('localhost', :worker_2_port);


-- remove a node with reference table with no active placement
...
-- test COPY then removing a node in a transaction
...
BEGIN;
COPY remove_node_reference_table FROM STDIN;
1
2
3
4
